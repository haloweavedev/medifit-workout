...
function getExerciseData(exerciseName) {
    const exercises = {
        'pull-ups': {
            title: 'Pull-Ups',
            videoUrl: 'https://placehold.co/640x360/4a90e2/ffffff?text=Pull-Ups+Video',
            instructions: [
        ...
        'push-ups': {
            title: 'Push-Ups',
            videoUrl: 'https://placehold.co/640x360/28a745/ffffff?text=Push-Ups+Video',
            instructions: [
        ...
        'dumbbell-rows': {
            title: 'Dumbbell Rows',
            videoUrl: 'https://placehold.co/640x360/dc3545/ffffff?text=Dumbbell+Rows+Video',
            instructions: [
        ...
        'shoulder-press': {
            ...
            videoUrl: 'https://placehold.co/640x360/fd7e14/ffffff?text=Shoulder+Press+Video',
            instructions: [
        ...
        'squats': {
            title: 'Squats',
            videoUrl: 'https://placehold.co/640x360/6f42c1/ffffff?text=Squats+Video',
            instructions: [
        ...
        'lunges': {
            title: 'Lunges',
            videoUrl: 'https://placehold.co/640x360/20c997/ffffff?text=Lunges+Video',
            instructions: [
        ...
        'romanian-deadlifts': {
            ...
            videoUrl: 'https://placehold.co/640x360/e83e8c/ffffff?text=Romanian+Deadlifts+Video',
            instructions: [
        ...
        'calf-raises': {
            title: 'Calf Raises',
            videoUrl: 'https://placehold.co/640x360/6c757d/ffffff?text=Calf+Raises+Video',
            instructions: [
        ...
        'planks': {
            title: 'Planks',
            videoUrl: 'https://placehold.co/640x360/17a2b8/ffffff?text=Planks+Video',
            instructions: [
    ...
    return exercises[exerciseName] || {
        title: 'Exercise Video',
                    videoUrl: 'https://placehold.co/640x360/000000/ffffff?text=Exercise+Video',
...
// Navigate to workout page with specific program
function navigateToWorkout(programId) {
    window.location.href = `workout.html?program=${programId}`;
}

// Handle exercise card clicks to navigate to workout page
function initializeExerciseCardNavigation() {
    const exerciseCards = document.querySelectorAll('.exercise-card');
    
    exerciseCards.forEach(card => {
        // Add click handler to the entire card (except the video button)
        card.addEventListener('click', function(e) {
            // Don't navigate if clicking on the video button
            if (e.target.closest('.btn') || e.target.closest('.play-button')) {
                return;
            }
            
            // Get the exercise name from the card
            const exerciseName = this.querySelector('h4')?.textContent?.toLowerCase().replace(/\s+/g, '-');
            const programType = determineWorkoutProgram(exerciseName);
            
            // Navigate to workout page with appropriate program
            navigateToWorkout(programType);
        });
...
// Determine workout program based on exercise name
function determineWorkoutProgram(exerciseName) {
    // Map exercises to appropriate programs
    const exerciseToProgram = {
        'pull-ups': 'muscle-gain-5-beginner',
        'push-ups': 'muscle-gain-5-beginner',
        'dumbbell-rows': 'muscle-gain-5-beginner',
        'shoulder-press': 'muscle-gain-5-beginner',
        'squats': 'muscle-gain-5-beginner',
        'lunges': 'muscle-gain-5-beginner',
        'romanian-deadlifts': 'muscle-gain-5-beginner',
        'calf-raises': 'muscle-gain-5-beginner',
        'planks': 'muscle-gain-5-beginner',
        'hiit': 'hiit-5-beginner',
        'cardio': 'fat-loss-3-beginner',
        'mobility': 'mobility-3-beginner',
        'flexibility': 'mobility-3-beginner'
    };
    
    // Default to muscle gain program
    return exerciseToProgram[exerciseName] || 'muscle-gain-5-beginner';
}

// Initialize exercise card navigation on archive page
document.addEventListener('DOMContentLoaded', function() {
    if (document.querySelector('.programs-grid')) {
        initializeExerciseCardNavigation();
    }
});

// Handle program card clicks on archive page
function initializeProgramCardNavigation() {
    const programCards = document.querySelectorAll('.program-card');
    ...
    programCards.forEach(card => {
        card.addEventListener('click', function(e) {
            ...
            // Get program data from card attributes
            const level = this.dataset.level;
            const duration = this.dataset.duration;
            const type = this.dataset.type;
            
            // Generate program ID
            const programId = generateProgramId(type, duration, level);
            
            // Navigate to workout page
            navigateToWorkout(programId);
        });
...
// Generate program ID from card data
function generateProgramId(type, duration, level) {
    const typeMap = {
        'mobility': 'mobility',
    ...
    const mappedType = typeMap[type] || 'muscle-gain';
    const mappedDuration = durationMap[duration] || '5';
    const mappedLevel = level.toLowerCase();
    
    return `${mappedType}-${mappedDuration}-${mappedLevel}`;
}

// Initialize program card navigation on archive page
document.addEventListener('DOMContentLoaded', function() {
    if (document.querySelector('.programs-grid')) {
        initializeProgramCardNavigation();
    }
});// Mobile Navigation Toggle
document.addEventListener('DOMContentLoaded', function() {
    const hamburger = document.querySelector('.hamburger');
